...
from utils1 import worker_init_fn, my_global_add_pool
...
class Net(torch.nn.Module):
    def __init__(self, args):
        ...
        #self.node_emb = Embedding(7, 126)
        self.edg_emb = Embedding(10, 3)
        self.embedding1 = nn.Linear(3, 3)
        ...
        for _ in range(1):
            conv = PNAConv(in_channels=3, out_channels=3,
        ...
        self.mlp = Sequential(Linear(3, 1))
...
def prepare_io_data(Y, config):
    ...
    if os.path.exists(data_save_path):
        ...
        for idx in tqdm(range(config.length)):
            ...
            X = [atom_feature(f1, k) for k in range(natoms)]
            # print(X)
            '''
